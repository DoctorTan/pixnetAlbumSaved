...
let photoLinks
(async () => {
  let config = await readAlbumUrlFromLocalConfig()
  //获取从本地读取的文件
  pixnetAlbumNameUrl = config.pixnetAlbumNameUrl
  // console.log(config.pixnetAlbumNameUrl);
...
async function getAllPicUrl(url) {
  ...
  const dir = path.join(__dirname, albumName);
...
function readAlbumUrlFromLocalConfig() {
